...
if ~exist('/mnt/galaxyData/tmp/sge')
  username = whoami ;
  base_dir = '/mnt/galaxyData/tmp/sge' ; %sprintf('%s/sge/tmp/%s',home_str, username) ;
  if exist(base_dir)~=7,
...
myassert(exist('/mnt/galaxyData/tmp/sge')==7) ;
...
elseif isequal(use_engine, 'octave'),
  ...
  elseif isequal(environment, 'galaxy'),
    bin_str = '/mnt/galaxyTools/tools/oqtans/sw_require/octave/bin/octave' ;
  else
...
mat_fname=sprintf('/mnt/galaxyData/tmp/sge/%s.mat',prefix) ;
result_fname=sprintf('/mnt/galaxyData/tmp/sge/%s_result.mat',prefix) ;
m_fname=sprintf('/mnt/galaxyData/tmp/sge/%s.m',prefix) ;
...
if isequal(use_engine, 'matlab'),
  ...
  elseif isequal(environment, 'galaxy'),
    evalstring_=['addpath(''/mnt/galaxyTools/tools/oqtans/sw_require/utils/rproc'') ;'] ;
    evalstring_=[evalstring_ 'addpath(''/mnt/galaxyTools/tools/oqtans/sw_require/utils'') ;'] ;
  else
    error('unknown environment') ;
  end ;
end 
...
if options.immediately,
  str=[ envstr 'cd /mnt/galaxyData/tmp/; cat ' m_fname '| ' bin_str ' >>' log_fname ] ;
elseif options.immediately_bg,
  str=[ envstr 'cd /mnt/galaxyData/tmp/; cat ' m_fname '| ' bin_str  ' >>' log_fname '&' ] ;
else
  % start matlab
  str=['echo "' envstr 'hostname; export LD_LIBRARY_PATH=/mnt/galaxyTools/tools/oqtans/sw_require/octave/syslib; cd /mnt/galaxyData/tmp/; cat ' m_fname '| ' ...
       bin_str ' >>' log_fname '" | /opt/sge/bin/lx24-amd64/qsub -o "' qsublog_fname '" -j y -r y ' ...
